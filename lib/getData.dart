...
/*
List<String> p2=[];
class getFeedblogs extends StatelessWidget {
  final String postIDss;
  getFeedblogs({required this.postIDss});
  
  @override
  Widget build(BuildContext context) {
    CollectionReference posts=FirebaseFirestore.instance.collection('blogs');

    return FutureBuilder<DocumentSnapshot>(
      future: posts.doc(postIDss).get(),
      builder: ((context, snapshot) {
         if(snapshot.connectionState==ConnectionState.done){
            ...
            return Column(
              children: [
                //Container(child: Image.network('${data['image']}')),
                Container(child: Text(data['content'])),
                Container(child: Text('Uploaded By: ${data['email']}'),),
                Container(child: Text('Caption: ${data['Caption']}'),),
              ],
            );
